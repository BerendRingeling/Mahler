# Figure 6.2 -- Histogram of the 2-adic valuations v_2(n_k) for 1 <= k <= 250K.
#
# Data source:  output_full.txt  (r_k = Fraction(num, den);  n_k = 1/r_k, so
#               v_2(n_k) = v_2(den) - v_2(num)).
#
# Displays the figure (plt.show); nothing is written to disk.
# Run:  sage plot_v2_histogram.sage      (or notebook / python3)
import os, re, glob
import numpy as np
